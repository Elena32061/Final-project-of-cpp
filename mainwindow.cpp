...
//希望显示在MainWidget之上的窗口，要在初始化MainWidget后再初始化
    initialFightWin();
//StoreWidget只是一个类，要想有主窗口，类中必须自己定义一个
...
void MainWindow::initialControlWidget()
{
    ...
    GenerateButton->setText("creat maze");
    AutoMoveButton->setText("find road");
    AIAnimationButton->setText("AI operation");
...
void MainWindow::initialinfoWidget()
{
    ...
    QFont fontLabel("Microsoft YaHei" ,6, 75);
    QFont fontNum("Microsoft YaHei" ,8, 65);
    QFont fontName("Microsoft YaHei" ,8, 55);
    QString button_style=
    "QPushButton{border-image:url(:/interface/image/interface/labelbg.png);color:white;border-radius:10px;}"                     
    ...
    label_Name->setStyleSheet(
        "border-image: url(:/interface/image/interface/labelbg.png);"
        );

    CharacterName->setFont(fontName);
    CharacterName->setStyleSheet(
        "border-image: url(:/interface/image/interface/labelbg.png);"
        );
    label_level->setFont(fontLabel);
    label_level->setStyleSheet(
        "border-image: url(:/interface/image/interface/labelbg.png);"
        );
    label_hp->setFont(fontLabel);
    label_hp->setStyleSheet(
        "border-image: url(:/interface/image/interface/labelbg.png);"
        );
    label_mp->setFont(fontLabel);
    label_mp->setStyleSheet(
        "border-image: url(:/interface/image/interface/labelbg.png);"
        );
    label_atk->setFont(fontLabel);
    label_atk->setStyleSheet(
        "border-image: url(:/interface/image/interface/labelbg.png);"
        );
    label_def->setFont(fontLabel);
    label_def->setStyleSheet(
        "border-image: url(:/interface/image/interface/labelbg.png);"
        );
    ...
    label_exp->setStyleSheet(
        "border-image: url(:/interface/image/interface/labelbg.png);"
        );
    label_floor1->setFont(fontLabel);
    label_floor1->setStyleSheet(
        "border-image: url(:/interface/image/interface/labelbg.png);"
        );
    label_floor2->setFont(fontLabel);
    label_floor2->setStyleSheet(
        "border-image: url(:/interface/image/interface/labelbg.png);"
        );
    ...
    label_hp->setText("HP");
    label_mp->setText("MP");
    label_atk->setText("atk");
    label_def->setText("def");
    label_score->setText("coin");
    ...
    label_floor1->setText("number");
    label_floor2->setText("floor");
    ReturnMainMenu->setText("Mainmeun");
...
void MainWindow::initialGetitemWin()
{
    ...
    QFont font("Microsoft YaHei" ,6, 75);
...
void MainWindow::CreateMaze_Layout()
{
    ...
    {
        QMessageBox message(QMessageBox::NoIcon, "！", "input 6-58");
...
void MainWindow::ShowMaze_Layout()
{
    ...
    for(int i=0;i<m.height;i++)
    {
        for(int j=0;j<m.width;j++)
        {
            ...
            if(m.Matrix[i][j].state == 1)//状态1 代表 路 非障碍
...
void MainWindow::QuestionBox(int num)
{
    ...
    QMessageBox message(QMessageBox::Information,"found box！","box with question symbol,Do you open it?",QMessageBox::Yes|QMessageBox::No,this);
    ...
    message.setWindowIcon(QIcon(":/info/image/information/提示.ico"));
        if (message.exec()==QMessageBox::Yes)
        {
            ...
            case 11:
            {
                QMessageBox mes(QMessageBox::NoIcon, "you open a box", "a green duck attack at you\n,you need to fight it");
            ...
            case 12:
            {
                QMessageBox mes(QMessageBox::NoIcon, "you open a box\n", "a red duck attack at you,\nyou need to fight it!");
                mes.setWindowIcon(QIcon(":/info/image/information/提示.ico"));
...
void MainWindow::checkPrefloor()
{
    if(d.floor==1)
    {
        QMessageBox message(QMessageBox::NoIcon, "sorry！", "you can't get out of the Dungeon！");
...
void MainWindow::checkNextfloor()
{
    ...
    {
        QMessageBox message(QMessageBox::NoIcon, "What do you want？", "This is the end of the Dungeon！");
...
void MainWindow::keyPressEvent(QKeyEvent *event)
{
    if(surface->classicalisok)//经典模式
    {
        ...
        {
            QMessageBox message(QMessageBox::NoIcon, "congratulations！", "Enter the next level of the maze！");
    ...
    else if(surface->Dungeonisok)//地牢模式
    {
        ...
        //keyevent 是实时检测的，只要在战斗结束后再设置isFighting为false，就可以了
        {
            return;
        }
...
/********************************************************************************************************/
          case Qt::Key_W://上
            ...
            //余江南改
             else if(d.x>0&&d.map[d.floor-1][d.x-1][d.y]==41)
            {
                Monsters(7,0);
            }
            //余江南改
...
/********************************************************************************************************/
        case Qt::Key_S://下
            ...
            //余江南改
             else if(d.x<11&&d.map[d.floor-1][d.x+1][d.y]==41)
            {
                Monsters(7,1);
            }
            //
...
/********************************************************************************************************/
        case Qt::Key_A://左
            ...
            //余江南改
             else if(d.y>0&&d.map[d.floor-1][d.x][d.y-1]==41)
            {
                Monsters(7,2);
            }
            //
...
/********************************************************************************************************/
        case Qt::Key_D:
            ...
            //余江南改
            else if(d.y<15&&d.map[d.floor-1][d.x][d.y+1]==41)
            {
                Monsters(7,3);
            }
...
void MainWindow::GetitemWinshow()
{
    if(GetitemNum==0)
    {
        ...
        case -1:
        {
            GetitemsShow->setText("you can't beat it!");
            GetitemsShow->show();
        }
            break;
        case -2:
        {
            GetitemsShow->setText("Monster defense is too high,\n you need to increase attack power");
            GetitemsShow->show();
        }
            break;
        case -3:
        ...
        case 0:
        {
            GetitemsShow->setText("You can't beat this monster, \nso you have to escape");
            GetitemsShow->show();
        }
         break;
        case 1:
        {
            GetitemsShow->setText("Your health is too low, \nyou need to improve to defeat the target! ");
            GetitemsShow->show();
        }
            break;
        case 8:
        {
            GetitemsShow->setText("Get a small blood-recovery mushroom! \nHealth increases by 100 ");
            GetitemsShow->show();
        }
            break;
        case 9:
        {
            GetitemsShow->setText("Get the blue mushroom!\n Mana value increases by 50");
            GetitemsShow->show();
        }
            break;
        case 10:
        {
            GetitemsShow->setText("Get big mushroom！\nMana value increases by 100");
            GetitemsShow->show();
        }
            break;
        case 11:
        {
            GetitemsShow->setText("a green duck attacking you，\nyou need to fight");
            GetitemsShow->show();
        }
            break;
        case 12:
        {
            GetitemsShow->setText("a red duck attacking you，\nyou need to fight");
            GetitemsShow->show();
        }
        case 16:
        {
            GetitemsShow->setText("can not open,\n need red key");
            GetitemsShow->show();
        }
            break;
        case 17:
        ...
        case 19:
        {
            GetitemsShow->setText("get a red key");
            GetitemsShow->show();
        }
            break;
        case 20:
        {
            GetitemsShow->setText("get a yellow key");
            GetitemsShow->show();
        }
            break;
        case 21:
        {
            GetitemsShow->setText("get a purple key");
            GetitemsShow->show();
        }
            break;
        case 22:
        {
            GetitemsShow->setText("get green ，\nadd three def");
            GetitemsShow->show();
        }
            break;
        case 23:
        {
            GetitemsShow->setText("get red ，\nadd 2 atk");
            GetitemsShow->show();
        }
            break;
        case 24:
        {
            GetitemsShow->setText("get sliver sword ，\nadd  10 atk");
            GetitemsShow->show();
        }
            break;//一定要break;不然会接着执行后面的语句
        case 28:
        {
            GetitemsShow->setText("get flower sword ，\nadd  25 atk");
            GetitemsShow->show();
        }
            break;
        case 29:
        {
            GetitemsShow->setText("get golden sword ，\nadd  36 atk");
            GetitemsShow->show();
        }
            break;
        case 35:
        {
            GetitemsShow->setText("you need more money");
            GetitemsShow->show();
        }
            break;
    }
...
void MainWindow::FightWinshow()//战斗界面
{
    ...
    //fight_period_it 代表对战次数,
    //奇数次计算任务对怪物造成的伤害，偶数次计算怪物对人造成的伤害
    else if (fight_period_it % 2 == 1 && fight_end_it == 0)
    {
        ...
        {
            Monster_hp->setText(QString::number(0));
            //fight_period_it / 2 + 1 代表人物对怪物攻击的累计次数
            fight_end_it = 1;
        }
        else
        {
    ...
    else
    {
        ...
        else if(fight_end_it == 3)
        {
            ...
            //隐藏战斗界面
            FightWidget->hide();
            GetitemsShow->setText(QString::fromWCharArray(L"get exper ") + QString::number(d.enemy[MonsterNum].exp) + QString::fromWCharArray(L" money ") + QString::number(d.enemy[MonsterNum].gold));
            GetitemsShow->show();
            sound->stop();
